...
public class Game : NetworkInterface
{
    ...
    public GameObject m_Main_Spawn_StartGameBtnDebug;
    ...
    void LateUpdate()
    {
        this.m_FPS = 1.0f / (Time.unscaledDeltaTime == 0 ? 1.0f : Time.unscaledDeltaTime);
    }
    ...
    public void On_ReceivedPlayerLoadingPercentage(VRCPlayerApi requestingPlayer, float percentage, string playerNameId, float fps)
    {
        ...
        if (Time.time - this.m_LastUpdatedLoadingPercentageTimeText > 1.0f)
        {
            ...
            foreach (VRCPlayerApi player in allPlayers)
            {
                ...
                this.m_PlayerGenStatusText += $"{player.displayName}: ({this.m_PlayerLoadingPercentages[key]}%) FPS: {thisFPS}\n";
    ...
    public void EnableGameStartButton()
    {
        if (Networking.IsMaster)
        {
            if (this.m_Main_Spawn_StartGameBtnDebug == null)
            {
                Debug.LogError("[Game.cs] HandleGameStateWaiting: m_Main_Spawn_StartGameBtn is null");
                return;
            }
            if(!this.m_Main_Spawn_StartGameBtnDebug.activeSelf)
            {
                this.m_Main_Spawn_StartGameBtnDebug.SetActive(true);
            }
    ...
    public void On_AnnounceStartGameButtonPressed(VRCPlayerApi requestingPlayer)
    {
        ...
        // when the master presses the start game button, do an animation or something idk
    }
    ...
    public void GenerateNewMaze()
    {
        ...
        {
            Debug.Log("[Game.cs] GenerateNewMaze: Successfully generated maze");
    ...
    public void On_GameEndedLordDied()
    {
        if(!Networking.IsMaster) return;
        if(this.m_GameStatus != GameStatus.InProgress) return;
        if(!this.m_LocalPlayerInGame) return;
    ...
    [NetworkedMethod]
    public void On_GameFinished(VRCPlayerApi requestingPlayer, bool lordDied, bool allNonFollowersDied)
    {
        if(requestingPlayer == null) return;
        if(!requestingPlayer.IsValid()) return;
        if(!requestingPlayer.isMaster) return;
